   ...
   <a name="top"> </a>
    <div id="full">
     ...
     <div id="about"> <marquee><h4> Welcome Admin <?php echo "$userid";?> Now You can Manage DATA </h4></marquee></div>
      <div id="te"> <center><h3> Available Teachers </h3>
      <?php
           echo "<center><table>";
           ...
           while($row1=$result->fetch_array())
            {
              $a=1;
              echo "<tr><td>$row1[2] $row1[3] $row1[4]</td><td> $row1[5]</td><td> $row1[6]</td><td>$row1[7] $row1[8] $row1[9]</td><td>$row1[10]</td><td><form action='delete_back.php' method='post' ><input type='text' value='$row1[0]' style='display:none' name='id'><input type='submit'value='Delete Teacher'></a></form></td></tr>";
            }
            echo "</table></center>";
            if($a==0)
             {
